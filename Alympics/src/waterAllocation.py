    ...
    def _parse_result(self, round_info):
        ...
        while attempts < 3:
            try:
                ...
                logger.error(f"JSON解析失败 (尝试 {attempts+1}/3): {e} \n原始内容: {res}") # 打印出来方便调试
                attempts += 1
                time.sleep(1) # 稍微歇一秒再试
        
        # 如果3次都失败了，为了不让程序崩溃，返回一个默认的字典（全0）
        logger.error("严重错误：无法解析 LLM 返回的出价，使用默认值 0 处理。")
        return {p.name: 0 for p in self.survival_players}
