...
class AgentPlayer(Player):
    ...
    def __init__(self, name, persona, engine):
        self.name = name
        self.engine = engine # 注意：我们会把这个变量传给 model 参数
        self.hp = 10
    ...
    def parse_result(self, message):
        ...
        while status != 1:
            try:
                ...
                times+=1
                if times>=3:
                    print("Too many parsing errors, exiting...")
                    exit()
            except Exception as e:
                print(e)
                time.sleep(15)
...
"""
    ...
    INQUIRY_SPP = ("Ok, {name}! Now is the ROUND {round}, and your HP is at {hp}. "
                   "Please choose an integer between 1 and 100 for this round. "
                   "Now, identify the participants and collaboratively choose the number step by step. Remember to provide the final solution with the following format \" The chosen number here.\".")
